"""
SemEval 2026 Task 4 Subtask 1 - v9
Self-consistency with native Gemini 2.5 Flash (k=5),
route to GPT-4.1 via OpenRouter for split votes.
...
MODEL_PRO = "openai/gpt-4.1"
DATA_PATH = "data/dev_track_a.jsonl"
OUTPUT_PATH = "data/v9_results.csv"
...
async def predict_with_gpt41(row: dict, votes: list[str], http_client: httpx.AsyncClient, semaphore: asyncio.Semaphore) -> tuple[str, str]:
    """Get decision from GPT-4.1 via OpenRouter for split votes."""
    ...
    async with semaphore:
        ...
        if votes:
            return Counter(votes).most_common(1)[0][0], "GPT-4.1 failed, using majority vote"
...
async def predict_cascade(row: dict, http_client: httpx.AsyncClient, semaphore: asyncio.Semaphore) -> dict:
    ...
    pro_decision, pro_reasoning = await predict_with_gpt41(row, valid_votes, http_client, semaphore)
    return {
...
async def main():
    print(f"{'═'*60}")
    print(f"SemEval 2026 Task 4 Subtask 1 - v9")
    print(f"Fast Model: {MODEL_FAST} (native API)")
    print(f"Pro Model: {MODEL_PRO} (OpenRouter)")
