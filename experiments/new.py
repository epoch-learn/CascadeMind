"""
SemEval 2026 Task 4 Subtask 1 - v10 (new.py)
Self-consistency with native Gemini 2.5 Flash (k=5),
route to GPT-5-mini via OpenRouter for split votes.

Strategy:
1. Run k=5 samples with Gemini 2.5 Flash (native API)
2. If unanimous → use that answer
3. If split → route to GPT-5-mini for final decision
"""
...
AZURE_DEPLOYMENT = "gpt-5-mini"  # Update with your deployment name
AZURE_API_VERSION = "2025-04-01-preview"

MODEL_FAST = "gemini-2.5-flash"
MODEL_PRO = "gpt-5-mini (Azure)"
...
def build_pro_prompt(row: dict, votes: list[str]) -> str:
    """Enhanced prompt for GPT-4.1 on split votes."""
...
async def predict_with_pro(row: dict, votes: list[str], http_client: httpx.AsyncClient, semaphore: asyncio.Semaphore) -> tuple[str, str]:
    """Get decision from GPT-5-mini via Azure OpenAI for split votes."""
    ...
    async with semaphore:
        for attempt in range(3):
            try:
                ...
                print(f"Azure API error (attempt {attempt+1}): {e}")
                if attempt < 2:
                    await asyncio.sleep(2 ** attempt)
        
        # Fallback to majority vote
        if votes:
            return Counter(votes).most_common(1)[0][0], "GPT-5-mini failed, using majority vote"
...
async def predict_cascade(row: dict, http_client: httpx.AsyncClient, semaphore: asyncio.Semaphore) -> dict:
    ...
    # Split votes or not enough votes - route to GPT-5-mini
...
async def main():
    print(f"{'═'*60}")
    print(f"SemEval 2026 Task 4 Subtask 1 - v10 (new.py)")
    print(f"Fast Model: {MODEL_FAST} (native API)")
    print(f"Pro Model: {MODEL_PRO}")
    print(f"Strategy: Self-consistency (k={NUM_SAMPLES}) → GPT-5-mini for splits")
    print(f"{'═'*60}")
    ...
    print(f"Routed to GPT-5-mini: {len(routed_df)} cases → {routed_correct}/{len(routed_df)} = {routed_correct/len(routed_df):.1%}" if len(routed_df) > 0 else "No routing needed")
    print(f"{'═'*60}")
    ...
    if len(routed_df) > 0:
        ...
        print(f"\n💭 Sample GPT-5-mini reasoning (example {sample_idx + 1}):")
        print(f"{'─'*60}")
